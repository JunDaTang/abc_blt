-- ============================================================
-- 存储过程: p_abc_fct_rr_dist
-- 功能: RR（资源→资源）分摊，将共享资源成本按动因量比例分摊到各功能中心
--       共享资源（如场地租金、水电费）需要在收端(1010)、派端(1020)、运输(1030)、管理(1040)等功能中心间分配
-- 输入参数: p_to_dt - 截至日期（默认当前日期），用于确定分摊所属月份
-- 输入表: abc_dim_dept（机构维表）、abc_rel_rr_dist（RR分摊规则）、
--         abc_fct_reso_list（资源清单-原始资源金额）、abc_fct_rr_driv（RR动因量）
-- 输出表: abc_fct_rr_dist（RR分摊结果）、abc_fct_rr_dist_tmp01/02/03（中间临时表）
-- 执行顺序: 第6步-RR分摊（四级分摊模型第1级：资源→资源）
-- ============================================================
...
DROP PROCEDURE IF EXISTS p_abc_fct_rr_dist;
DELIMITER //
CREATE PROCEDURE p_abc_fct_rr_dist(IN p_to_dt DATE)
BEGIN
  ...
  SET v_sqlstate  = '变量赋值';
  SET v_proc_name = 'p_abc_fct_rr_dist';
  ...
  SET v_sqlstate = '删除数据';
  DELETE FROM abc_fct_rr_dist_tmp01;
  DELETE FROM abc_fct_rr_dist_tmp02;
  DELETE FROM abc_fct_rr_dist_tmp03;
  DELETE FROM abc_fct_rr_dist a WHERE a.month_code = v_month;

  -- ============================================================
  -- 第1步(tmp01): 建立分摊标准
  -- 将分摊规则表(abc_rel_rr_dist)与机构维表(abc_dim_dept)做展开
  -- 按机构类型匹配，生成所有「源功能中心→目标功能中心」的分摊关系行
  -- 每行代表一条待分摊的路径：fm(源功能中心) → to(目标功能中心)
  -- ============================================================
  ...
  INSERT INTO abc_fct_rr_dist_tmp01
    SELECT b.mode_code,
           ...
           b.fm_dept_type_name,
           b.fm_func_code,                      -- 源功能中心(1010收端/1020派端/1030运输/1040管理)
           b.fm_func_name,
           b.fm_reso_code,                      -- 源资源代码(如ZY0101人工)
           b.fm_reso_name,
           ...
           b.to_func_name,
           b.to_reso_code,                      -- 目标资源代码(分摊后归属的资源科目)
           b.to_reso_name,
           b.dist_type,                         -- 分摊方式
  ...
  -- ============================================================
  -- 第2步(tmp02): 关联资源
  -- 将分摊标准与资源清单(abc_fct_reso_list)关联，匹配每条分摊路径对应的资源金额(fm_amt)
  -- 通过源机构+源功能中心+源资源代码定位资源金额
  ...
  INSERT INTO abc_fct_rr_dist_tmp02
    SELECT a.mode_code,
           ...
           a.fm_reso_code,
           a.fm_reso_name,
           b.car_no            fm_car,          -- 车辆编号(运输资源的区分维度)
           b.amt               fm_amt,          -- 资源原始金额(待分摊总额)
           a.to_dept_code,
           a.to_dept_name,
           a.to_dept_type_code,
           a.to_dept_type_name,
      ...
      LEFT JOIN abc_fct_reso_list b
        ON a.fm_dept_code = b.dept_code        -- 源机构匹配
       AND a.fm_func_code = b.func_code        -- 源功能中心匹配
       AND a.fm_reso_code = b.reso_code        -- 源资源代码匹配
       AND b.month_code = v_month;             -- 月份匹配

  -- ============================================================
  -- 第3步(tmp03): 关联动因
  -- 将上一步结果与RR动因表(abc_fct_rr_driv)关联，获取每个目标功能中心的动因量(qty)
  -- 同时用窗口函数计算每个「源」的总动因量(all_qty)，用于后续按比例分摊
  -- 窗口函数分区键 = 源机构+源功能中心+源资源+车辆，即同一笔资源的所有分摊目标
  -- ============================================================
  SET v_sqlstate = '关联动因';
  INSERT INTO abc_fct_rr_dist_tmp03
    SELECT a.mode_code,
           ...
           a.driv_name,
           b.qty,                                                          -- 该目标功能中心的动因量
           SUM(b.qty) OVER(PARTITION BY a.fm_dept_code, a.fm_func_code, a.fm_reso_code, a.fm_car) all_qty  -- 总动因量(分摊标准合计)
  ...
  -- ============================================================
  -- 第4步: 生成分摊结果，写入最终表 abc_fct_rr_dist
  -- 核心分摊公式:
